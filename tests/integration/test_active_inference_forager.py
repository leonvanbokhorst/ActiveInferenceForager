...
def test_integration(
    conversation_manager, user_model, fep_predictor, active_inference_agent
):
    ...
    # Predict next topic
    user_model.update_preference("AGI", 0.9)
    next_topic = fep_predictor.predict_next_topic()
    assert next_topic == "AGI"

    user_model.update_preference("OPENAI", 0.7)
    user_model.add_interaction("user input", "ai response", "AI")
    next_topic = fep_predictor.predict_next_topic()
    assert next_topic == "AGI"
    ...
    user_model.add_interaction("user input", "ai response", "AGI")
    next_topic = fep_predictor.predict_next_topic()
    assert next_topic == "AI"
